(() => {
  ...
  const SOURCE_URL = 'src/loader-v26.js?v=33';
  const API_MARKER = 'window.RBTwarAPI={';
  const VALIDATION_MARKER = '      try {\n        new Function(code);';

  function fail(error) {
    console.error('RBTwar v33 no pudo preparar la progresión:', error);
    const toast = document.getElementById('toast');
    const btn = document.getElementById('startBtn');
    if (btn) { btn.disabled = true; btn.textContent = 'ERROR'; }
    if (toast) { toast.textContent = 'Error al preparar la progresión de niveles.'; toast.classList.add('show'); }
  }

  console.info('RBTwar bootstrap v33');
  ...
  fetch(SOURCE_URL, { cache:'no-store' })
    ...
    .then(loader => {
      ...
      // RBTwar v33: niveles continuos, ciclos procedurales y dificultad acotada.
      code = replaceRequired(
        code,
        "const STORAGE_KEY='rbtwar-save-v26',MAX_LEVEL=30;",
        "const STORAGE_KEY='rbtwar-save-v33',MAX_LEVEL=Number.MAX_SAFE_INTEGER;",
        'guardado continuo v33'
      );
      code = code.replace(
        "for(const key of [STORAGE_KEY,'rbtwar-save-v25'",
        "for(const key of [STORAGE_KEY,'rbtwar-save-v26','rbtwar-save-v25'"
      );
      code = replaceRequired(
        code,
        ...
        'títulos y mundos continuos v33'
      );
      code = replaceRequired(
        code,
      ...
      code = replaceRequired(
        code,
        "ui.startProgress.textContent=\`\${save.unlockedLevel} de \${MAX_LEVEL} niveles desbloqueados\`;",
...
`;
      ...
      if (!loader.includes('niveles continuos')) throw new Error('No se pudo insertar la progresión v33.');

      try { new Function(loader); }
      catch (e) { throw new Error(`Sintaxis bootstrap v33: ${e.message}`); }

      const script = document.createElement('script');
      script.dataset.rbtwarBootstrap = 'v33';
