(() => {
  ...
  function showError(error) {
    console.error('RBTwar v20 no pudo iniciar:', error);
  ...
  function replaceBetween(source, startMarker, endMarker, replacement, label) {
    ...
    if (start < 0) throw new Error(`No se encontró inicio: ${label}`);
    const end = source.indexOf(endMarker, start + startMarker.length);
    if (end < 0) throw new Error(`No se encontró final: ${label}`);
    return source.slice(0, start) + replacement + '\n' + source.slice(end);
  }

  console.info('RBTwar bootstrap v20');
  ...
  fetch(ENGINE_URL, { cache: 'no-store' })
    ...
    .then(source => {
      ...
      code = code.replace("const STORAGE_KEY='rbtwar-save-v7',MAX_LEVEL=30;","const STORAGE_KEY='rbtwar-save-v20',MAX_LEVEL=30;");
      code = code.replace("for(const key of [STORAGE_KEY,'rbtwar-save-v6'","for(const key of [STORAGE_KEY,'rbtwar-save-v19','rbtwar-save-v18','rbtwar-save-v12','rbtwar-save-v11','rbtwar-save-v10','rbtwar-save-v9','rbtwar-save-v8','rbtwar-save-v7','rbtwar-save-v6'");
...
function deselect(){selectedSquadIds.clear();if(game)game.squads.forEach(s=>s.selected=false);selectedSquadId=null;ui.selectionPanel.classList.add('hidden');}`;
      code = replaceBetween(code,'function selectSquad(s){','function commandRoad(',selectionFunctions,'selección múltiple');
 ...
 ctx.restore();
      ...
      code = replaceBetween(code,'function drawRange(){','function commandPoint(',drawRange,'rangos múltiples');
 ...
 if(selectionBox){const x=Math.min(selectionBox.x1,selectionBox.x2),y=Math.min(selectionBox.y1,selectionBox.y2),w=Math.abs(selectionBox.x2-selectionBox.x1),h=Math.abs(selectionBox.y2-selectionBox.y1);ctx.save();ctx.fillStyle='rgba(58,134,255,.10)';ctx.strokeStyle='rgba(120,185,255,.92)';ctx.lineWidth=2*scale();ctx.setLineDash([7*scale(),5*scale()]);ctx.fillRect(x,y,w,h);ctx.strokeRect(x,y,w,h);ctx.restore();}
      ...
      code = replaceBetween(code,'function drawCommand(){','function drawCastle(',drawCommand,'visual de órdenes');

      const targetAndGroupFunctions = `function playerSquadAt(p,exclude=null,radiusPx=42){
...
function targetAt(p,s,excluded=new Set()){
 const ally=game.squads.filter(a=>a.team===TEAM.PLAYER&&a.hp>0&&a.id!==s.id&&!excluded.has(a.id)).map(a=>({s:a,d:dist(p,a)})).filter(x=>x.d<=82*scale()).sort((a,b)=>a.d-b.d)[0]?.s||null;
 if(ally)return{kind:'ally',squad:ally};
 const n=nodeAt(p);if(n)return{kind:'node',node:n};
 const snap=snapToRoad(p.x,p.y);if(snap&&snap.dist<=62*scale())return{kind:'road',snap};return null;
}
function snapAtProgress(base,progress){
...
function executeGroup(ids,t){
 ...
 if(t.kind==='node'){list.forEach((s,i)=>{commandNode(s,t.node);if(list.length>1&&s.route.length){const last=s.route.at(-1);if(last){const angle=(Math.PI*2*i)/list.length,r=10*scale()*Math.min(3,list.length);last.x+=Math.cos(angle)*r;last.y+=Math.sin(angle)*r;}}});return true;}
...
function squadsNearNode(n,radius=155){return game.squads.filter(s=>s.team===TEAM.PLAYER&&s.hp>0&&dist(s,n)<=radius*scale());}`;
      code = replaceBetween(code,'function playerSquadAt(','function execute(',targetAndGroupFunctions,'selección y órdenes de grupo');

      const pointerFunctions = `function onDown(e){
 if(!game||inMenu||paused||ended)return;e.preventDefault();const p=pointerPos(e);pointer=p;const squad=playerSquadAt(p,null,42),node=!squad?nodeAt(p):null;
...
function onUp(e){
 ...
 drag=null;selectionBox=null;
      ...
      code = replaceBetween(code,'function onDown(','function showToast(',pointerFunctions,'gestos táctiles');

      const autoMerge = `function ordersCompatible(a,b){
 if(!a.route.length&&!b.route.length)return true;
 const ae=a.route.at(-1)||a.order,be=b.route.at(-1)||b.order;
 if(ae&&be&&Math.hypot((ae.x||a.x)-(be.x||b.x),(ae.y||a.y)-(be.y||b.y))<=95*scale())return true;
 return Boolean((a.order?.kind==='ally'&&a.order.targetId===b.id)||(b.order?.kind==='ally'&&b.order.targetId===a.id));
}
function mergeNearby(){
 for(let i=0;i<game.squads.length;i++)for(let j=i+1;j<game.squads.length;j++){
  const a=game.squads[i],b=game.squads[j];
  if(a.hp<=0||b.hp<=0||a.team!==b.team||a.type!==b.type||a.combatTargetId||b.combatTargetId||(a.count>=5&&b.count>=5))continue;
  const d=dist(a,b);if(d>86*scale()||!ordersCompatible(a,b))continue;
  if(d<=52*scale()){const target=a.count>=b.count?a:b,mover=target===a?b:a;mergePair(mover,target,false);continue;}
  const pull=Math.min(1.5*scale(),Math.max(.20,(d-52*scale())*.04)),dx=(b.x-a.x)/Math.max(1,d),dy=(b.y-a.y)/Math.max(1,d);
  a.x+=dx*pull;a.y+=dy*pull;b.x-=dx*pull;b.y-=dy*pull;
 }
}`;
      code = replaceBetween(code,'function mergeNearby(){','function updateParticles(',autoMerge,'unión automática');
 ...
 for(const id of [...selectedSquadIds])if(!game.squads.some(s=>s.id===id))selectedSquadIds.delete(id);
 if(selectedSquadIds.size)refreshSelectionPanel();else{selectedSquadId=null;ui.selectionPanel.classList.add('hidden');}
}`;
      code = replaceBetween(code,'function cleanup(){','function checkDefeat(',cleanup,'limpieza de selección');
      code = code.replace("canvas.addEventListener('pointercancel',()=>drag=null);","canvas.addEventListener('pointercancel',()=>{drag=null;selectionBox=null;});");
