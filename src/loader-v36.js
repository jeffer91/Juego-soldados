(() => {
  ...
  const SOURCE_URL = 'src/loader-v26.js?v=36';
  const API_MARKER = 'window.RBTwarAPI={';
  const VALIDATION_PATTERN = /\n\s*try\s*\{\s*\n\s*new Function\(code\);/;

  function fail(error) {
    console.error('RBTwar v36 no pudo preparar el motor:', error);
    const toast = document.getElementById('toast');
    const btn = document.getElementById('startBtn');
    if (btn) { btn.disabled = true; btn.textContent = 'ERROR'; }
    if (toast) { toast.textContent = 'Error al preparar RBTwar v36.'; toast.classList.add('show'); }
  }

  console.info('RBTwar bootstrap v36');
  ...
  fetch(SOURCE_URL, { cache:'no-store' })
    ...
    .then(rawLoader => {
      ...
      if (!VALIDATION_PATTERN.test(loader)) throw new Error('Punto de validación del motor no encontrado.');

      const progressionPatch = `
      // RBTwar v36: niveles continuos, balance, sonido de batalla y limpieza de unidades huérfanas.
      code = replaceRequired(
        code,
        "const STORAGE_KEY='rbtwar-save-v26',MAX_LEVEL=30;",
        "const STORAGE_KEY='rbtwar-save-v36',MAX_LEVEL=Number.MAX_SAFE_INTEGER;",
        'guardado continuo v36'
      );
      code = code.replace(
        "for(const key of [STORAGE_KEY,'rbtwar-save-v25'",
        "for(const key of [STORAGE_KEY,'rbtwar-save-v35','rbtwar-save-v34','rbtwar-save-v33','rbtwar-save-v26','rbtwar-save-v25'"
      );
      code = replaceRequired(
        code,
        ...
        'títulos y mundos continuos v36'
      );
      code = replaceRequired(
        code,
      ...
      code = replaceRequired(
        ...
        "if(n.captureProgress>=2.6){const previousTeam=n.team;n.team=team;game.individuals=game.individuals.filter(u=>!(u.homeNode===n.index&&u.team===previousTeam));n.spawnQueue=0;",
        'limpieza al capturar fábrica'
      );
      code = replaceRequired(
        code,
        "n.captureProgress=0;n.captureTeam=null;spawnBurst(n.x,n.y,TEAM_COLOR[team],10);",
      ...
      code = replaceRequired(
        ...
        "else if(t.hp<=0){t.hp=0;window.dispatchEvent(new CustomEvent('rbtwar:base_destroyed',{detail:{attackerTeam:p.team,destroyedTeam:t.team,nodeId:t.id}}));if(p.team===TEAM.PLAYER)winLevel();else loseLevel();}",
        'evento de caída del CORE'
      );
      code = replaceRequired(
        code,
        "function winLevel(){if(ended)return;ended=true;",
        "function winLevel(){if(ended)return;ended=true;window.dispatchEvent(new CustomEvent('rbtwar:victory',{detail:{level:game.level}}));",
        'evento de victoria'
      );
      code = replaceRequired(
        code,
        "function loseLevel(){if(ended)return;ended=true;",
        "function loseLevel(){if(ended)return;ended=true;window.dispatchEvent(new CustomEvent('rbtwar:defeat',{detail:{level:game.level}}));",
        'evento de derrota'
      );
`;

      loader = loader.replace(VALIDATION_PATTERN, match => '\n' + progressionPatch + match);
...
`;
      ...
      if (!loader.includes('evento de disparo')) throw new Error('No se pudo insertar el audio de batalla.');
      if (!loader.includes('niveles continuos')) throw new Error('No se pudo insertar la progresión v36.');

      try { new Function(loader); }
      catch (e) { throw new Error(`Sintaxis bootstrap v36: ${e.message}`); }

      const script = document.createElement('script');
      script.dataset.rbtwarBootstrap = 'v36';
