(() => {
  ...
  const reasonText={core_full:'CORE al máximo',no_squads:'Necesitas un pelotón',already:'Ya está activo',inactive:'No disponible ahora'};
  let used = new Set();
  ...
  function canReward(kind){
    if(used.has(kind))return{ok:false,reason:'already'};
    if(rewardsThisRun>=2)return{ok:false,reason:'limit'};
    try{return window.RBTwarAPI?.canReward?.(kind) || {ok:true};}catch(_){return{ok:false,reason:'inactive'};}
  }
  ...
  function openPanel(){
    ...
    for(const kind of Object.keys(rewards))if(canReward(kind).ok)analytics('offer',kind);
  }
  ...
  function renderOptions(){
    ...
    for(const [kind,item] of Object.entries(rewards)){
      const availability=canReward(kind);
      ...
      let disabledText='';
      if(used.has(kind))disabledText='Ya usado en esta partida';
      else if(rewardsThisRun>=2)disabledText='Límite alcanzado';
      else disabledText=reasonText[availability.reason]||'No disponible ahora';
      btn.innerHTML=`<span class="assist-icon">${item.icon}</span><span><b>${item.title}</b><small>${unavailable?disabledText:item.desc}</small></span><em>${unavailable?'—':'🎬 VER'}</em>`;
  ...
  async function requestReward(kind){
    ...
    const precheck=canReward(kind);
    if(!precheck.ok){renderOptions();toast(reasonText[precheck.reason]||'Esa ayuda no está disponible ahora.');return;}
    busy=true;analytics('click',kind);closePanel(false);
    const secondCheck=canReward(kind);
    if(!secondCheck.ok){busy=false;renderOptions();resumeAfterPanel();toast(reasonText[secondCheck.reason]||'Esa ayuda ya no está disponible.');return;}
    analytics('started',kind);
    try{
      const rewarded=await showProvider(kind);
      if(!rewarded){analytics('skipped',kind);return;}
      analytics('completed',kind);
      const finalCheck=canReward(kind);
      if(!finalCheck.ok){analytics('failed',kind);toast('La situación cambió; no se consumió la ayuda.');return;}
  ...
  function syncButton(){
    ...
    const panel=$('assistPanel');if(panel&&!panel.classList.contains('hidden'))renderOptions();
  }
  ...
  function init(){
    ...
    console.info(`RBTwar recompensas v34 listas · ${DEV_ADS?'anuncios de prueba':'proveedor nativo'}`);
  }
