(() => {
  ...
  const SOURCE_URL = 'src/loader-v26.js?v=35';
  const API_MARKER = 'window.RBTwarAPI={';
  const VALIDATION_PATTERN = /\n\s*try\s*\{\s*\n\s*new Function\(code\);/;

  function fail(error) {
    console.error('RBTwar v35 no pudo preparar el motor:', error);
    const toast = document.getElementById('toast');
    const btn = document.getElementById('startBtn');
    if (btn) { btn.disabled = true; btn.textContent = 'ERROR'; }
    if (toast) { toast.textContent = 'Error al preparar RBTwar v35.'; toast.classList.add('show'); }
  }

  console.info('RBTwar bootstrap v35');
  ...
  fetch(SOURCE_URL, { cache:'no-store' })
    ...
    .then(rawLoader => {
      ...
      // RBTwar v35: niveles continuos, ciclos, balance y economía estables.
      code = replaceRequired(
        code,
        "const STORAGE_KEY='rbtwar-save-v26',MAX_LEVEL=30;",
        "const STORAGE_KEY='rbtwar-save-v35',MAX_LEVEL=Number.MAX_SAFE_INTEGER;",
        'guardado continuo v35'
      );
      code = code.replace(
        "for(const key of [STORAGE_KEY,'rbtwar-save-v25'",
        "for(const key of [STORAGE_KEY,'rbtwar-save-v34','rbtwar-save-v33','rbtwar-save-v26','rbtwar-save-v25'"
      );
      code = replaceRequired(
        code,
        ...
        'títulos y mundos continuos v35'
      );
      code = replaceRequired(
        code,
...
`;
      ...
      if (!loader.includes('niveles continuos')) throw new Error('No se pudo insertar la progresión v35.');

      try { new Function(loader); }
      catch (e) { throw new Error(`Sintaxis bootstrap v35: ${e.message}`); }

      const script = document.createElement('script');
      script.dataset.rbtwarBootstrap = 'v35';
